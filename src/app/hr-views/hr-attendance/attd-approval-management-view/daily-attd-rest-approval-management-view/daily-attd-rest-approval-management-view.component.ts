...
@Component({
  selector: 'app-daily-attd-rest-approval-management-view',
  templateUrl: './daily-attd-rest-approval-management-view.component.html',
  styleUrls: ['./daily-attd-rest-approval-management-view.component.css']
})
export class DailyAttdRestApprovalManagementViewComponent implements OnInit {
...
//조회버튼을 누르면
  ...
  // alert(this.approvalStatus);
   ...
   this.hrService.transaction("hr/attendance/findAttdRestListByCondition.do","","approvalStatus="+this.approvalStatus+" basicDay="+this.selectDate+" deptCode="+this.deptCode).subscribe((result : any[])=>{
  ...
  checkApproval(){
    ...
    this.agGrid.api.getSelectedNodes().forEach(element => {
       //  alert(element.rowIndex);
         this.statmentArray["DailyAttdRestList"][element.rowIndex].status="updated"

        if(status!="nomal") {
          if(this.statmentArray["DailyAttdRestList"][element.rowIndex].approvalStatus=="N"){
         this.statmentArray["DailyAttdRestList"][element.rowIndex].approvalStatus="Y"
          }else{
            this.statmentArray["DailyAttdRestList"][element.rowIndex].approvalStatus="N"
          }
        }
       // alert(this.statmentArray["DailyAttdRestList"][element.rowIndex].status);
      //  alert(this.statmentArray["DailyAttdRestList"][element.rowIndex].approvalStatus);
       // alert(this.statmentArray["DailyAttdRestList"][element.rowIndex].cost);
...
checkdelete(){
  ...
  this.agGrid.api.getSelectedNodes().forEach(element => {
   // alert(element.rowIndex);
    this.statmentArray["DailyAttdRestList"][element.rowIndex].status="deleted"
   this.agGrid.api.redrawRows();  //리로드
  });
      ...
      savebtn(){
       ...
       // alert(batchData);
        this.hrService.transaction("hr/attendance/batchDailyAttdRest.do",JSON.stringify(batchData),"").subscribe();
        this.agGrid.api.redrawRows();  //리로드
...
//0붙이기
 ...
 columnDefs=[
  ...
  {headerName: '일수', field: 'days', width: 60},
